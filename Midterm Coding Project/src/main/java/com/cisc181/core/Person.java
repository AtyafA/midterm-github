...
public abstract class Person implements java.io.Serializable {
	...
	public void setDOB(Date DOB) throws PersonException {  
	 Calendar calendarDOB = new GregorianCalendar();
    calendarDOB.setTime(DOB);
    if (GregorianCalendar.getInstance(Locale.getDefault()).get(Calendar.YEAR)
            - calendarDOB.get(Calendar.YEAR) > 100)
        throw new PersonException(this, "Wrong date");
		this.DOB = DOB;
	...
	public void setPhone(String newPhoneNumber) throws PersonException{
	Pattern pattern = Pattern.compile("[(]\\a\\a\\a[)]-\\a\\a\\a-\\a\\a\\a\\a");
    Matcher matcher = pattern.matcher(newPhoneNumber);
    if (matcher.find()){
		phone_number = newPhoneNumber;}
		  else 
              throw new PersonException(this, "Wrong phone format");
